...
from libs.dist_utils import *
from libs.lmdb_writer import *
from libs.metrics import *
from libs.renderer import *
from libs.diffusion.base import _extract_into_tensor
...
class LitModel(pl.LightningModule):
	def __init__(self, conf: TrainConfig):
		...
		self.model = conf.make_model_conf().make_model() # BeatGANsAutoencModel
		...
		self.T_sampler = conf.make_T_sampler(define_t = 50) # Define T not 1000 but 50 for stochastic encoding
	...
	def sample(self, N, device, T=None, T_latent=None):
		if T is None:
			...
			sampler = self.conf._make_diffusion_conf(T).make_sampler() # -> self.diffusion_type = 'beatgans', self.beatgans_gen_type = GenerativeType.ddim
		...
		noise = torch.randn(N,
							...
							self.conf.img_size,
							device=device)
		# print('x_t', noise.shape)
		pred_img = render_uncondition(   # -> renderer.py (TrainMode.latent_diffusion)
			self.conf,
			self.ema_model,
			noise,
			sampler=sampler,
	...
	def render(self, noise, cond=None, T=None, shift_h = None, return_all_steps = False, train_diff = False, model = None):
		...
		if cond is not None:
			pred_img = render_condition(self.conf,
										...
										noise,
										sampler=sampler,
										cond=cond, shift_h = shift_h, return_all_steps = return_all_steps)
		else:
	...
	def render_inter(self, noise, z_edit, z_source, T=None):
		if T is None:
			sampler = self.eval_sampler
		else:
			sampler = self.conf._make_diffusion_conf(T).make_sampler()

		pred_img = render_condition_inter(self.conf,
										self.ema_model,
										noise,
										sampler=sampler,
										z_edit=z_edit, z_source = z_source)
		
		return pred_img

	def encode(self, x, return_2d_feature = False):		
		assert self.conf.model_type.has_autoenc()
		if not return_2d_feature:
			cond = self.ema_model.encoder.forward(x, return_2d_feature = return_2d_feature)
			return cond
		else:
			cond, cond_2d = self.ema_model.encoder.forward(x, return_2d_feature = return_2d_feature)
			return cond, cond_2d 
	
	def encode_flatten(self, z_2d):
		cond = self.ema_model.encoder.forward_flatten(z_2d)
		return cond
		

	def encode_stochastic(self, x, cond, T=None, return_h_space = False, return_all = False, model = None):
		if T is None:
			sampler = self.eval_sampler
		else:
			sampler = self.conf._make_diffusion_conf(T).make_sampler()
		...
		out = sampler.ddim_reverse_sample_loop(model, # base.py
											   x, return_h_space = return_h_space,
											   model_kwargs={'cond': cond})
		if return_h_space:
			return out['sample'], out['h_space']
		if return_all:
			return out
		else:
			return out['sample']

	def encode_stochastic_blend(self, x, cond, cond2, T=None, return_h_space = False, return_all = False):
		if T is None:
			sampler = self.eval_sampler
		else:
			sampler = self.conf._make_diffusion_conf(T).make_sampler()

		out = sampler.ddim_reverse_sample_loop(self.ema_model, # base.py
											   x, return_h_space = return_h_space,
											   model_kwargs={'cond': cond2, 'cond2': cond})
		if return_h_space:
			return out['sample'], out['h_space']
		if return_all:
			return out
	...
	def encode_stochastic_one_step(self, x, cond, T=None,  return_all = False, device = 'cuda'):
		if T is None:
			sampler = self.eval_sampler
		else:
			sampler = self.conf._make_diffusion_conf(T).make_sampler()
		...
		sample = x
		indices = list(range(sampler.num_timesteps))
		# t = th.tensor([indices[-1]] * len(sample), device=device)
		t = th.tensor([1] * len(sample), device=device)
		# print(t)
		# quit()
		out = self.sampler.ddim_reverse_sample(self.model,
										sample,
										t=t,
										model_kwargs={'cond': cond}
										)
		...
		# eps = (_extract_into_tensor(self.sampler.sqrt_recip_alphas_cumprod, t, x.shape)
		# 	   * x - out["pred_xstart"]) / _extract_into_tensor(
		# 		   self.sampler.sqrt_recipm1_alphas_cumprod, t, x.shape)
		# alpha_bar_next = _extract_into_tensor(self.sampler.alphas_cumprod_next, t,
		# 									  x.shape)
