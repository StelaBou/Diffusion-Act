...
from libs.model.unet_autoenc import AutoencReturn
from libs.config_base import BaseConfig
import enum
import math
...
from libs.model.nn import mean_flat
from typing import NamedTuple, Tuple
from libs.choices import *
...
class GaussianDiffusionBeatGans:
	...
	def sample(self,
			   ...
			   clip_denoised=True,
			   model_kwargs=None,
			   progress=False, shift_h = None, z_source = None, return_all_steps = False):
		"""
		Args:
		...
		# print(self.conf.gen_type) --> GenerativeType.ddim
		...
		if self.conf.gen_type == GenerativeType.ddpm:
			...
			return self.ddim_sample_loop(model,
										 ...
										 clip_denoised=clip_denoised,
										 model_kwargs=model_kwargs,
										 progress=progress, shift_h = shift_h, z_source = z_source, return_all_steps = return_all_steps)
		else:
			raise NotImplementedError()
	...
	def p_mean_variance(self,
						...
						clip_denoised=True,
						denoised_fn=None,
						model_kwargs=None, return_h_space = False, shift_h = None):
		"""
		...
		with autocast(self.conf.fp16):
			if return_h_space:
				model_forward, h_space = model.forward(x=x, # unet_autoenc
										  t=self._scale_timesteps(t), return_h_space = return_h_space, shift_h = shift_h,
										  **model_kwargs)
			else:
	...
	def ddim_sample(
		...
		out = self.p_mean_variance(
			...
			clip_denoised=clip_denoised,
			denoised_fn=denoised_fn,
			model_kwargs=model_kwargs, shift_h = shift_h
		)
		...
		# #     print(out.keys())
		#     # print(out['pred_xstart'].shape)
		#     # print(sample.shape)
		#     # print(t[0])
		# save_image(out['pred_xstart'], os.path.join('./test_results/pred_xstart_{:02d}.png'.format(t[0].cpu().numpy())))
		# save_image(sample, os.path.join('./test_results/sample_{:02d}.png'.format(t[0].cpu().numpy())))
			# quit()
		return {"sample": sample, "pred_xstart": out["pred_xstart"]}
	...
	def ddim_reverse_sample(
		...
		out = self.p_mean_variance(
			...
			clip_denoised=clip_denoised,
			denoised_fn=denoised_fn,
			model_kwargs=model_kwargs, return_h_space = return_h_space
		)
		# Usually our model outputs epsilon, but we re-derive it
		# in case we used x_start or x_prev prediction.
		...
		if return_h_space:
			return {"sample": mean_pred, "pred_xstart": out["pred_xstart"], "h_space": out["h_space"]}
		else:
	...
	def ddim_reverse_sample_loop(
		...
		model_kwargs=None,
		eta=0.0,
		device=None, return_h_space = False,
	):
		...
		for i in indices:
			...
			# if 'cond2' in model_kwargs and t > 20:
			# 	model_kwargs['cond'] = model_kwargs_copy['cond2']

			# print('ddim_reverse_sample_loop')
			with th.no_grad():
				# save_image(sample, os.path.join('./demos/editing_test/noisy_{:02d}_edit.png'.format(t[0].cpu().numpy())))
				out = self.ddim_reverse_sample(model,
												...
												denoised_fn=denoised_fn,
												model_kwargs=model_kwargs,
												eta=eta, return_h_space = return_h_space)
				sample = out['sample']
				# [1, ..., T]
				...
				# save_image(sample, os.path.join('./demos/editing_test/noisy_{:02d}_edit.png'.format(t[0].cpu().numpy())))
				# save_image(out['pred_xstart'], os.path.join('./demos/editing_test/noisy_x0{:02d}_edit.png'.format(t[0].cpu().numpy())))
				# [0, ..., T-1] ready to use
				T.append(t)
				if return_h_space:
					h_space = out['h_space']
	...
	def ddim_sample_loop(
		...
		device=None,
		progress=False,
		eta=0.0, shift_h = None, z_source = None, return_all_steps = False,
	):
		"""
		...
		for sample in self.ddim_sample_loop_progressive(
				...
				device=device,
				progress=progress,
				eta=eta, shift_h = shift_h, z_source = z_source
		):
			final = sample
			if return_all_steps:
	...
	def ddim_sample_loop_progressive(
		...
		device=None,
		progress=False,
		eta=0.0, shift_h = None, z_source = None,
	):
		"""
		...
		# z_edit = model_kwargs['cond'].clone()
		...
		for i in indices:
			...
			t = th.tensor([i] * len(img), device=device)
			# print('ddim_sample_loop_progressive') # torch.no_grad() here
			# with th.no_grad():
			# if count != len(indices) - 1:
			# 	shift_h = None	
			if z_source is not None:
				
				v = ((self.num_timesteps-1) - i)/self.num_timesteps
				# z_t = v * z_source + (1-v) * z_edit
				z_t = z_edit
				# print(v, torch.sum(torch.abs(z_t)), torch.sum(torch.abs(z_edit)), torch.sum(torch.abs(z_source)))
				model_kwargs['cond'] = z_t
			# quit()
			out = self.ddim_sample(
				...
				eta=eta, shift_h = shift_h
			)
			out['t'] = t
			yield out
			img = out["sample"]
			
			# save_image(out['pred_xstart'], os.path.join('./demos/test_images/pred_xstart_{:02d}_edit.png'.format(t[0].cpu().numpy())))
			# save_image(img, os.path.join('./demos/test_images/sample_{:02d}_edit.png'.format(t[0].cpu().numpy())))
